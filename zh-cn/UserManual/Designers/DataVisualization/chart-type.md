...
交叉表（Cross Table / Pivot Table）是一种将数据按多个维度组合并汇总展示的表格组件，支持将不同字段作为 **行维度** 和 **列维度** 进行灵活组合，并在其交叉区域展示相应的指标字段汇总值。通过交叉表，用户可以清晰、直观地查看不同维度组合下的业务数据，便于洞察趋势、发现异常、辅助决策。

+ 维度配置
  - **行维度**：用于纵向分类展示，如「省份 > 城市」结构
  - **列维度**：用于横向分类展示，如「产品类别 > 产品名称」
  - 支持多个字段组合、重命名、升降序排序
...
+ 数值字段配置
  ...
  - 默认聚合方式：**求和（sum）**
  - 支持聚合方式切换：
    * 无处理、最小值、最大值、平均值、求和、计数、去重计数、样本/总体标准差与方差
+ 样式——数值展示格式：
  - **空值处理**：显示为 null / 0 / 自定义文本 / 不显示
  - **展示名称**：可自定义重命名
  - **数据格式**：标准数值 / 千分位 / 百分比 / 自定义单位
  - **小数位数**：支持设置精度（如保留2位小数）
+ 样式——数值位置样式
  - **数值作为列展示**：每个指标字段为一列，常用于维度字段少、指标字段较多的场景
  - **数值作为行展示**：指标字段作为额外行展示，适用于横向空间受限场景
+ 样式——合计与组内合计
  - **合计显示**：可设置显示行合计、列合计
  - **组内合计**：支持设置为前 N-1 个维度为分组依据（如省份 > 城市，可按省份合计）
    * 移除维度后，合计层级自动上移
  - **自定义合计算法**：支持 `sum`、`avg`、`min`、`max`、`count`、`count_distinct`
