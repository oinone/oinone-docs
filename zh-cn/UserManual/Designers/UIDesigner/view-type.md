...
2. 排序方式：表格提供两种设置排序的方式，它们的生效范围不同：
   + 单字段排序（表头设置）
     ...
     - 生效逻辑： 排序仅对当前点击的字段生效。这是一种快速的单级排序方式，会覆盖当前已应用的所有排序规则（包括弹窗中设置的多字段排序）。 完成配置后，点击“确定”按钮应用多级排序。
   + 多字段排序（图标弹窗设置）
     - 操作位置： 点击表格右上角的排序图标。
     - 生效逻辑： 可对多个字段设置排序规则，并支持手动拖拽调整优先级（排在越上方的字段，优先级越高）。
3. 删除排序
