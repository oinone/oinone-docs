...
更多方言配置参考：[数据库方言配置专题](/zh/DevManual/CommonSolutions/Back-End/data-dialect-opengauss-database-deployment.md)
...
**无公网访问地址配置**：MINIO 无公网访问地址下 OSS 的配置方式，详见 [文件存储：MINIO无公网访问地址下OSS的配置](/zh/DevManual/CommonSolutions/Back-End/file-storage-minio-oss-configuration-without-public-ip.md)。
...
参考：事件配置中[消息队列连接配置](#4efefdc2)

### 2、搜索引擎Elastic配置

参考：增强模型中[搜索引擎配置](#f9c0bcd1)
